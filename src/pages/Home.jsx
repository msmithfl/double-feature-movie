...
import poster_placeholder from "../assets/poster-placeholder.png";
...
const Home = () => {
  ...
  const [isPosterSet, setIsPosterSet] = useState(false);
        ...
        {posterPaths.map((posterPath, index) => (
          ...
          >
            <Link
              to={
                isPosterSet
                  ? `https://www.themoviedb.org/movie/${movies[index]?.id}`
                  : ""
              }
              target={isPosterSet ? "_blank" : "_self"}
            >
              <img
